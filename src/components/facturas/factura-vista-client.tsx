            ...
            <div className="w-full max-w-4xl">
                <div className="flex items-center gap-4 no-print">
                    ...
                    <div className="ml-auto flex items-center gap-2">
                        <Button size="sm" onClick={handlePrint} variant="outline" disabled={isPrinting || factura.estado === 'Cancelada'}>
                ...
                <Card className="p-6 mt-4 printable-area">
                    <div className="flex justify-between items-start mb-8">
                        <div>
                            ...
                            <p className="text-destructive font-semibold">{factura.numero_factura_proveedor}</p>
                        </div>
                        <div className="text-right">
                            <p><span className="font-semibold">Fecha Emisión:</span> {formatUTCDate(factura.fecha_emision)}</p>
                            <p><span className="font-semibold">Fecha Vencimiento:</span> {formatUTCDate(factura.fecha_vencimiento)}</p>
                            <Badge variant={getBadgeVariant(factura.estado)} className="mt-2">{factura.estado}</Badge>
                        </div>
                    </div>
                    ...
                    <Table>
                        ...
                        <TableBody>
                            {detalles.length > 0 ? (
                                detalles.map((d) => (
                                    <TableRow key={d.id}>
                                        <TableCell>{productoMap.get(d.producto_id) || 'N/A'}</TableCell>
                                ...
                                <TableRow>
                                    <TableCell colSpan={5} className="h-24 text-center">No hay productos en esta factura.</TableCell>
                                </TableRow>
                             )}
                        </TableBody>
                    </Table>
                    ...
                    <div className="mt-8 border-t pt-4 text-sm text-muted-foreground">
                        <p><span className="font-semibold">Tipo de Pago:</span> {factura.tipo_pago}</p>
                    </div>
                </Card>
            </div>
