...
const getDisplayUser = (item: AuditoriaLog): string => {
  // Check if role is 'Administrador'
  if (item.nombre_rol?.toLowerCase() === 'administrador') {
    return 'Administrador';
  }
  // Also check if the authenticated user in details is 'Administrador'
  if (item.details?.usuario_autenticado?.toLowerCase() === 'administrador') {
    return 'Administrador';
  }
  // If there's a user ID, it's a logged-in user action, show their role.
      ...
      <div className="flex flex-col gap-4">
        <div className="flex items-center justify-between gap-4">
          <div className="relative flex-1">
            ...
            <Input
              placeholder="Filtrar por acción, tabla, usuario, etc..."
              value={filter}
              onChange={(e) => {
        ...
        <Card className="border shadow-sm rounded-lg overflow-hidden">
          {/* Header */}
          <div className="grid bg-muted/50 font-medium text-muted-foreground p-4" style={{ gridTemplateColumns }}>
            <div className="w-[50px]"></div>
            <div className="cursor-pointer" onClick={() => handleSort("timestamp")}>
          ...
          <Accordion type="single" collapsible className="w-full">
            {paginatedData.length > 0 ? (
               paginatedData.map((item) => (
                 <AccordionItem value={`item-${item.id}`} key={item.id} className="border-b last:border-b-0">
                   <AccordionTrigger className="hover:no-underline hover:bg-muted/50 p-0 text-sm font-normal">
                      <div className="grid w-full p-4 text-left" style={{ gridTemplateColumns }}>
                          <ChevronDown className="h-4 w-4 shrink-0 transition-transform duration-200" />
                          <div>{formatUTCDate(item.timestamp)}</div>
                          <div>
                            <Badge variant={getBadgeVariant(item.accion)} className={getBadgeClassName(item.accion)}>
                              {item.accion}
                            </Badge>
                          </div>
                          <div>{item.tabla}</div>
