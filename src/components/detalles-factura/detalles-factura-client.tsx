...
export default function DetallesFacturaClient({ factura, initialDetalles, productos, proveedores }: DetallesFacturaClientProps) {
    ...
    const handlePrint = () => {
        startPrinting(async () => {
          ...
          if (result.success) {
            toast({ title: "Estado actualizado", description: "La factura está lista para imprimir." });
            window.open(`/facturas/vista?factura_id=${factura.id}&autoprint=true`, '_blank');
          } else {
            toast({ title: "Error", description: result.message, variant: "destructive" });
          }
        });
        ...
        <>
            <div className="flex items-center gap-4">
                ...
                <div className="ml-auto flex items-center gap-2">
                    <Button onClick={handlePrint} variant="outline" size="sm" disabled={isPrinting || factura.estado === 'Cancelada'}>
